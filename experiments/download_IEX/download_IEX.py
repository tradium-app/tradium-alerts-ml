...
env = Env()
env.read_env('../../.env')
IEX_API_TOKEN = env('IEX_API_TOKEN')
...
def get_dates():
    ...
    IEX_date = date_today - timedelta(days = 90) # 30 trailing calendar days acc. to IEX docu
...
def get_tickers(path):
    ...
    base_url = 'https://sandbox.iexapis.com/beta/ref-data/iex/symbols?format=csv&token={}'
    df = pd.read_csv(base_url.format(IEX_API_TOKEN))
...
def asyncio_prep(date_str):
    ...
    base_url = 'https://sandbox.iexapis.com/beta/stock/{}/chart/date/{}?format=csv&token={}&chartIEXOnly=true'

    path = get_path(date_str)
    # tickers = get_tickers(path)
    tickers = ['UPWK', 'QQQ', 'FVRR']
    api_urls = [base_url.format(ticker, date_str, IEX_API_TOKEN) for ticker in tickers]
